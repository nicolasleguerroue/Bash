Une fois placé dans le dossier, ouvrez un terminal et tapez :

 - chmod +x install.sh

Enfin, executez le programme en faisant :

 - sudo ./install.sh

le programme va commencer par demander votre mot de passe. Une zone de saisie va apparaitre
Si vous ne voyez pas les caractères que vous tapez, c'est tout à fait normal.
en cas de doute de saisie, pressez plusieurs fois la touche "Retour en arrière" et saississez à nouveau le 
mot de passe. 
Validez avec le bouton "Entrée".
En cas d'absence de réseau, le programme va tourner en boucle en affichant "En attente de réseau".
Pour quitter le programme, appuyez sur "Ctrl"+"z" ou "Ctrl"+"c"
Une fois le réseau revenu, laissez-vous guider par le logiciel.


Auteur  : Nicolas Le Guerroué
Date    : 04/04/2019
Licence : GPL
