...
SetSize 800 800
sudo mkdir /tmp/Utils_$(date +%H)$(date +%M)$(date +%S)
...
if [[ ! -d $storage ]]; then #Si le dossier n'existe pas, on l'installe
	...
	echo -e "#Ajout des alias automatiques [$(GetTime)] - Nicolas LE GUERROUE 0xf88" >> $alias_file
	...
	for directories in $(ls $storage/$command_directory); do
		...
		if [ -d $path ]; then
			...
			for files_command in $(ls $path); do
				echo -e "alias $files_command='$path/$files_command' #0xf88" >> $alias_file #0xf88 est une clé pour retrouver les lignes du fichier bashrc modifiées par Utils
				echo -e "$files_command='$path/$files_command'" >> $resume_output
			done
		fi
	done
	echo -e "#Fin de l'ajout des alias automatiques 0xf88" >> $alias_file
	...
	if [[ $(hostname -I) != "" ]]; then #Si réseau, on installe zenity
		...
		echo -e "Installation de Snap..."
		sudo apt-get install -y snap 2>> $errors_output >> $standard_output
		echo -e "Installation de Snap : [ $green OK $default ]"
		echo -e ">>> Installation de Snap effectuée" >> $resume_output
		echo -e "Installation de Zenity..."
		sudo apt-get install -y zenity 2>> $errors_output >> $standard_output
		echo -e "Installation de Zenity : [ $green OK $default ]"
		echo -e ">>> Installation de Zenity effectuée" >> $resume_output
		echo -e "Installation de Tree..."
		sudo apt-get install -y tree 2>> $errors_output >> $standard_output
		echo -e "Installation de Tree : [ $green OK $default ]"
		echo -e "Installation de Hardinfo..."
		sudo apt-get install -y hardinfo 2>> $errors_output >> $standard_output
		echo -e "Installation de Hardinfo : [ $green OK $default ]"
		echo -e ">>> Installation de Tree effectuée" >> $resume_output
		echo -e "Installation de Zip..."
		sudo apt-get install -y zip 2>> $errors_output >> $standard_output
		echo -e "Installation de Zip : [ $green OK $default ]"
		echo -e ">>> Installation de Zip effectuée" >> $resume_output
		...
		#TextInfos "Readme.txt"  2>> $errors_output >> $standard_output 
		SetSize 800 600
		Print "<b>bibliothèque Utils</b> - $(GetTime)\n
...
<b>man utils</b> [documentation terminal] 
